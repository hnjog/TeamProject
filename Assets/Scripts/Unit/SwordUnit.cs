...
public class SwordUnit : MonoBehaviour {
	...
	[Header("이동 관련 변수")]
    public float swordUnitSpeed = 1f;                 //유닛 이동속도
    public Rigidbody2D swordUnitRb;                   //유닛의 리지드바디
    
    [Header("스텟 관련 변수")]
    public int swordUnitHp = 70;                      //유닛 체력
    public int swordUnitAttack = 10;                   //유닛 공격력
    public int swordUnitDepend = 7;                   //유닛 방어력
    public int swordUnitCost = 100;                     //유닛 코스트
	public int swordCoolTime = 10;
    ...
    IEnumerator Move()
    {
        transform.Translate(Vector3.right * swordUnitSpeed * Time.deltaTime);
